 ...
 *
 * @param  {String}  plainword A plaintext password.
 *
 * @return {Promise}           The promise resolves with a hashed String to be stored; rejects with CryptographyError.
 */
export const saltAndHash = plainword => {
	return bcrypt.hash(plainword, saltRounds).catch(
		e => {
			if(isDebug){
				e.message += `; saltAndHash() received plainword=${plainword}.`;
				throw e;
 ...
 */
export const isMatch = (plainword, stored) => {
	return bcrypt.compare(plainword, stored).catch(
		e => {
			if(isDebug){
				e.message += `; isMatch() received plainword=${plainword} and stored=${stored}.`;
				throw e;
			}else throw new CryptographyError('The contents of the error are hidden for security reasons.');
		}
	);
